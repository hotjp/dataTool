    ...
    <div class="full">
        ...
        <div class="grid-content full">
          <div class="chart_data_info">
            ...
            <p class="table_name">{{charts.tableName.name}}
              <a href="javascript:;" class="r" @click="workTableDialogVisible = true">
      ...
      <div class="chart_main">
        <div class="grid-content">
          ...
          <el-card class="display_container box-card" :style="{width:(screenWidth-520)+'px'}">
            <div class="full empty_container" v-if="!echartsShow && !tableShow" :style="{height:chartsHeight-50+'px',lineHeight:chartsHeight+'px'}">
              暂无数据
              <!-- {{chartsHeight}}{{screenWidth}} -->
            </div>
            <IEcharts class="full charts_container" v-if="echartsShow" theme="macarons" :style="{height:chartsHeight-50+'px'}" :notMerge="true" :resizable="charts.resizable" :option="charts.option" :loading="charts.loading" @ready="onChartReady"></IEcharts>
            <tableView class="full table_container" :myMessage="chartData" v-if="tableShow" :style="{height:chartsHeight-50+'px'}"></tableView>
          </el-card>
...
export default {
  data() {
    return {
      ...
      checkAllTexts: false,
      checkedTexts: [],
      texts: [{ name: 'name', text: '名称', type: 'String' }, { name: 'type', text: '类型', type: 'String' }, { name: 'status', text: '状态', type: 'String' }],
      isIndeterminateTexts: true,
      checkAllNums: false,
      checkedNums: [],
      nums: [{ name: 'qty', text: '数量', type: 'Number' }, { name: 'price', text: '价格', type: 'Number' }],
  ...
  methods: {
    ...
    getTopHeight() {
      ...
      this.chartsHeight = document.body.clientHeight - 150 - Dvalue;
    },
    //添加到维度
    ...
    handleCheckedDatesChange(value) {
      ...
      this.isIndeterminateDates = checkedCount > 0 && checkedCount < this.dates.length;
    },
    //      弹出框选择文本型
    ...
    handleCheckedTextsChange(value) {
      ...
      this.isIndeterminateTexts = checkedCount > 0 && checkedCount < this.texts.length;
    },
    //      弹出框选择数值型
    ...
    toggleSelectedItem(index) {
      ...
      this.$set(this.queryInfo.categoryColumns[index], 'selectedItemSwitch', val);
    },
    toggleSelectedChildren(index, indexItem) {
      let selectedChildrenSwitch = this.queryInfo.categoryColumns[index].selectedChildrenSwitch ? this.queryInfo.categoryColumns[index].selectedChildrenSwitch : -1;
      let val = -1;
      ...
      this.$set(this.queryInfo.categoryColumns[index], 'selectedChildrenSwitch', val);
    },
    ...
    toggleValueSelectedItem(index) {
      let selectedItemSwitch = this.queryInfo.valueColumns[index].selectedItemSwitch ? this.queryInfo.valueColumns[index].selectedItemSwitch : false;
      let val = !selectedItemSwitch;
      this.$set(this.queryInfo.valueColumns[index], 'selectedItemSwitch', val);
    },
    toggleValueSelectedChildren(index, indexItem) {
      let selectedChildrenSwitch = this.queryInfo.valueColumns[index].selectedChildrenSwitch ? this.queryInfo.valueColumns[index].selectedChildrenSwitch : -1;
      let val = -1;
      if (selectedChildrenSwitch == indexItem) {
        val = -1;
      } else {
      ...
      this.$set(this.queryInfo.valueColumns[index], 'selectedChildrenSwitch', val);
    },
    ...
    onClickSetValueItemSort(index, item) {
      ...
      for (var i = 0; i < that.queryInfo.valueColumns.length; i++) {
        if (that.queryInfo.valueColumns[i].type == 'Number') {
          if (that.queryInfo.valueColumns[i].name == that.queryInfo.valueColumns[index].name) {
    ...
    handleCheckedCategoryNumsChange(value) {
      ...
      this.isIndeterminateCategoryNums = checkedCount > 0 && checkedCount < this.nums.length;
    },
